...
- `CMakeLists.txt` discovers configured test-suite directories and builds each
  `.cc` or `.cpp` file as a standalone executable.
...
First activate the development environment and point `icclrun` at the InfiniCCL
source checkout:

```bash
conda activate ninetoothed
export INFINICCL_ROOT=/path/to/InfiniCCL
```

Then prepare a local `cluster.yaml`. For a single NVIDIA laptop GPU, use one
slot and expose device 0:
...
nodes:
  - ip: "localhost"
    ...
    backend_env:
      ...
      LD_LIBRARY_PATH: "/path/to/InfiniCCL-Test/install/nvidia/lib"
